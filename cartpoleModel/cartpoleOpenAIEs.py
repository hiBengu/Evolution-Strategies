        ...
        with torch.no_grad():
            while not done:
                ...
                if render:
                    self.env.render()
                total_reward += reward
        self.totReward += total_reward
        self.counter += 1
        if self.counter == 100:
            print("Average Reward: " + str(self.totReward/100))
            averageScores.append(self.totReward/100)
            self.counter = 0
            self.totReward = 0
        return total_reward
...
if __name__ == '__main__':
    ...
    print("Save all Averages!")
    with open('allAverages.pkl', 'wb') as f:
        pickle.dump(averageScores, f)
