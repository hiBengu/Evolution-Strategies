...
class TrainPacman():
    def __init__(self, startGen=1, device=torch.device('cpu'), std=0.02):
    ...
    def saveAllAverages(self):
        ...
        with open(f'allAveragesForSimpleGeneticStd{self.std}.pkl', 'wb') as f:
    ...
    def generateNextGen(self):
        ...
        for i in range(2):
            print(f"An integer will be chosen from 0 to {len(self.bestScores)}")
            chosenGen = random.randint(0, len(self.bestScores)-1)
            while chosenGen in chosenGens:
                chosenGen = random.randint(0, len(self.bestScores)-1)
            chosenGens.append(chosenGen)

        # print(chosenGens)

        for index,i in enumerate(self.bestScores):
            if index in chosenGens:
                net = Net()
        ...
        for i in range(len(parameterList[0])):
            chooseModel = random.randint(0,1)
            # print(chooseModel)
            if chooseModel == 0:
                meanParameter.append(parameterList[0][i])
            else:
                meanParameter.append(parameterList[1][i])
        ...
        for j in range(len(parameterList[0])):
            param = np.random.normal(meanParameter[j].detach() , self.std, 100)
